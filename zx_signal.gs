...
class zxSignal_main isclass zxSignal			// òî, ÷òî íå âàæíî äëÿ ñâÿçè ñ áóäêîé
{
Library  mainLib;
GSObject[] GSO;
...
public void UpdateState(int reason, int priority)  	// îáíîâëåíèå ñîñòîÿíèÿ ñâåòîôîðà, îñíîâíîé êóñîê ñèãíàëüíîãî äâèæêà
	...
	if(Type & ST_UNLINKED)
		...
		if(train_open and (Type & ST_ROUTER))			// çàïóñêàåì ìàðøðóòíûé (ñ ñèíèì)
			{
			...
			if((track_params[0])[0]=='+')
				{					// åñëè ïåðåä ñâåòîôîðîì åñòü ïîåçä, îòêðûâàåì â îáû÷íîì ïîðÿäêå
				CheckMySignal(false);
				CheckPrevSignals(false);
				}
			else
			...
			if(MainState == 19)
				...
				CheckPrevSignals(false);		// ïîèñê ñëåäóþùåãî ñâåòîôîðà
			}
...
public void UnlinkedUpdate(int mainstate)
	...
	if(Type & ST_PERMOPENED)
		...
		if(ex_sgn[1] or ex_sgn[6])
			...
			{		// ÿâëÿåòñÿ ïîâòîðèòåëüíûì, ò.ê. èìååò òîëüêî çåë¸íóþ ëèíçó
...
public void Switch_span2()		// ïîâåðíóòü ñâåòîôîð â ñòîðîíó ýòîãî ñâåòîôîðà
{
	Switch_span(true);
}
...
// ðîçæèã
	...
	if(Type & (ST_IN+ST_OUT+ST_ROUTER))	// ñòàíöèîííûé ñâåòîôîð
		{
	...
	if(Type & ST_IN)	// âõîäíîé. Ïàíåëü ïåðåãîíà.
		{
...
public void SetPropertyValue(string id, string val)
{
 	...
 	if(id == "private-name")
		...
		kbm_mode = LC.FindPossibleSgn(ex_sgn, ex_lins);			//  ãåíåðèðóåì ðîçæèã
		MainState = LC.FindSignalState(false, 0, ex_sgn, ab4, 0, train_open, shunt_open, false, 0);
 		}
 }
...
public void LinkPropertyValue(string id)
{
	...
	if(id=="station_delete")
		...
		string[] l_k_arr = GetLensKit();	// åñëè íàáîð ëèíç ïî-óìîë÷àíèþ

		if(lens_kit_n >= l_k_arr.size())
			lens_kit_n = 0;			// ñèëüíî áîëüøîé
...
public string GetContentViewDetails(void)
{
	...
	if((Type & ST_IN) and span_soup and span_soup.GetNamedTagAsBool("Inited",false) )	// âõîäíîé. Ïàíåëü ïåðåãîíà.
		{
...
int FindTypeByLens(bool[] ex_lins)
	...
	if(ex_lins[0] and ex_lins[8] and (ex_lins[1] or ex_lins[3]))	// ìàðøðóòíûé îáëàäàåò è ñèíèì, è çåë¸íûì(æ¸ëòûì)
		type1 = type1 + ST_ROUTER;
	else
		{
		if(!(ex_lins[0] and ex_lins[8]) and  !ex_lins[1] and !ex_lins[3] and !ex_lins[4]) // ñèíåêðàñíûå - íå ìàíåâðîâûå è îñíîâíûõ ëèíç íåò
...
// äîáàâëÿåì ñòàíöèþ ñâåòîôîðà
	...
	if(lens_kit_n < 0)						// ñîáñòâåííûé íàáîð ëèíç
		lens_kit = soup.GetNamedTag("lens_kit");
	else
		{
		string[] l_k_arr=	GetLensKit();				// åñëè íàáîð ëèíç ïî-óìîë÷àíèþ áåð¸ì åãî
		if(lens_kit_n < l_k_arr.size())
			lens_kit = l_k_arr[lens_kit_n];			// åñëè îí âîîáùå åñòü?
	...
	if(ex_sign_1=="")
		kbm_mode = LC.FindPossibleSgn(ex_sgn, ex_lins);			// åñëè ðîçæèã íå ñãåíåðèðîâàí, ãåíåðèðóåì
	else
		{
...
public Soup GetProperties(void)
	...
	if(!wrong_dir)
		...
		if(Type & (ST_UNLINKED|ST_PROTECT))
			retSoup.SetNamedTag("privateStateEx", LC.sgn_st[19].l.MainState   );	// äëÿ ñîâìåñòèìîñòè ñ z7
		else
			retSoup.SetNamedTag("privateStateEx", LC.sgn_st[MainState].l.MainState   );	// äëÿ ñîâìåñòèìîñòè ñ z7
		}
	else
